...
import {initialJobtitle, Jobtitle} from '../core/_models'
import clsx from 'clsx'
import {useListView} from '../core/ListViewProvider'
import {JobtitlesListLoading} from '../components/loading/JobtitlesListLoading'
import {createJobtitle, updateJobtitle} from '../core/_requests'
...
const JobtitleEditModalForm: FC<Props> = ({jobtitle, isJobtitleLoading}) => {
  const {setItemIdForUpdate} = useListView()
  const {refetch} = useQueryResponse()

  const [jobtitleForEdit] = useState<Jobtitle>({
    ...jobtitle,
    // avatar: Jobtitle.avatar || initialJobtitle.avatar,
    description: jobtitle.description || initialJobtitle.description,
    // position: jobtitle.position || initialJobtitle.position,
    name: jobtitle.name || initialJobtitle.name,
    code: jobtitle.code || initialJobtitle.code,
    status: jobtitle.status || initialJobtitle.status,
  })
  ...
  // const bankAvatarImg = toAbsoluteUrl(`/media/${jobtitleForEdit.avatar}`)

  const formik = useFormik({
    initialValues: jobtitleForEdit,
    validationSchema: editBankSchema,
    onSubmit: async (values, {setSubmitting}) => {
      setSubmitting(true)
      try {
        if (isNotEmpty(values.id)) {
          await updateJobtitle(values)
        } else {
  ...
  return (
    <>
      <form id='kt_modal_add_bank_form' className='form' onSubmit={formik.handleSubmit} noValidate>
        ...
        >
          ...
          <div className='fv-row mb-7'>
            ...
            <input
              ...
              autoComplete='off'
              disabled={formik.isSubmitting || isJobtitleLoading}
            />
        ...
        <div className='text-center pt-15'>
          ...
          <button
            ...
            disabled={isJobtitleLoading || formik.isSubmitting || !formik.isValid || !formik.touched}
          >
            <span className='indicator-label'>Submit</span>
            {(formik.isSubmitting || isJobtitleLoading) && (
      ...
      {(formik.isSubmitting || isJobtitleLoading) && <JobtitlesListLoading />}
    </>
  )
}

export {JobtitleEditModalForm}
