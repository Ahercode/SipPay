...
import {initialNotch, Notch} from '../core/_models'
import clsx from 'clsx'
import {useListView} from '../core/ListViewProvider'
import {NotchsListLoading} from '../components/loading/NotchsListLoading'
import {createNotch, updateNotch} from '../core/_requests'
import {useQueryResponse} from '../core/QueryResponseProvider'

type Props = {
  isNotchLoading: boolean
  notch: Notch
}

const editNotchSchema = Yup.object().shape({
  // email: Yup.string()
...
const NotchEditModalForm: FC<Props> = ({notch, isNotchLoading}) => {
  const {setItemIdForUpdate} = useListView()
  const {refetch} = useQueryResponse()

  const [notchForEdit] = useState<Notch>({
    ...notch,
    // avatar: Notch.avatar || initialNotch.avatar,
    status: notch.status || initialNotch.status,
    // position: Notch.position || initialNotch.position,
    name: notch.name || initialNotch.name,
    salarygrade: notch.salarygrade || initialNotch.salarygrade,
    annual: notch.annual || initialNotch.annual,
    monthly: notch.monthly || initialNotch.monthly,
  })
  ...
  // const NotchAvatarImg = toAbsoluteUrl(`/media/${NotchForEdit.avatar}`)

  const formik = useFormik({
    initialValues: notchForEdit,
    validationSchema: editNotchSchema,
    onSubmit: async (values, {setSubmitting}) => {
      setSubmitting(true)
      try {
        if (isNotEmpty(values.id)) {
          await updateNotch(values)
        } else {
          await createNotch(values)
        }
      } catch (ex) {
        console.error(ex)
      } finally {
  ...
  return (
    <>
      <form id='kt_modal_add_Notch_form' className='form' onSubmit={formik.handleSubmit} noValidate>
        {/* begin::Scroll */}
        <div
        ...
        >
          ...
          <div className='fv-row mb-7'>
            ...
            > */}
              ...
              {/* <div
                ...
                style={{backgroundImage: `url('${NotchAvatarImg}')`}}
              ></div> */}
          ...
          <div className='fv-row mb-7'>
            ...
            <input
              ...
              autoComplete='off'
              disabled={formik.isSubmitting || isNotchLoading}
            />
          ...
          <div className='fv-row mb-7'>
            ...
            {/* <input
              placeholder='salarygrade'
              {...formik.getFieldProps('salarygrade')}
              type='text'
              name='salarygrade'
              className={clsx(
                'form-control form-control-solid mb-3 mb-lg-0',
                {'is-invalid': formik.touched.salarygrade && formik.errors.salarygrade},
                {
                  'is-valid': formik.touched.salarygrade && !formik.errors.salarygrade,
                }
              )}
              autoComplete='off'
              disabled={formik.isSubmitting || isNotchLoading}
        ...
        <div className='text-center pt-15'>
          ...
          <button
            ...
            disabled={isNotchLoading || formik.isSubmitting || !formik.isValid || !formik.touched}
          >
            <span className='indicator-label'>Submit</span>
            {(formik.isSubmitting || isNotchLoading) && (
      ...
      {(formik.isSubmitting || isNotchLoading) && <NotchsListLoading />}
    </>
  )
}

export {NotchEditModalForm}
